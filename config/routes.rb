...
#  map.tournament '', :controller => "tournaments", :action => "index", :conditions => {:subdomain => /.+/}
  ...
  map.namespace(:admin) do |admin|
    ...
    admin.resources :schedules, :member => {:results => :get, :update_results => :post}, :collection => {:week => :get, :data => :get}
...
#  map.devise_for :users
  ...
  map.resources :users
  map.resources :pages
  map.resources :schedules#, :collection => {:show_quick_results => :get}
  ...
  map.with_options :conditions => {:subdomain => /.+/} do |tournament|
    ...
    tournament.resources :seasons do |season|
      season.resources :matches, :only => ["show", "index"]
    end
