namespace :export do
  ...
  task footballers_teams: :environment do
    CSV.open('db/export/footballers_teams.csv', 'w') do |csv|
      ...
      # create export csv
      FootballersTeam.order("footballers_teams.footballer_id ASC").each do |item|
        csv << [item.footballer_id, item.team_id, item.step_id]
      end
    end
  end

  task league_tags: :environment do

    CSV.open('db/export/league_tags.csv', 'w') do |csv|
      csv << %w{id step_season_id name created_at updated_at}

      # create export csv
      LeagueTag.find_each do |item|
        csv << [item.id, item.step_season_id, item.name, item.created_at, item.updated_at]
      end
    end
  end
