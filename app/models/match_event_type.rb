...
class MatchEventType < ActiveRecord::Base
  ...
  has_many :match_events do
    def add params
      ...
      params[:message] ||= proxy_association.owner.template.gsub(/(^:|[\b\s]:)(\w+)/){ s, v = $1, $2; s.gsub(':', '') + lambda{ |o| o.is_a?(ActiveRecord::Base) ? (o.to_tpl rescue ":#{v}") : o.to_s }[params.delete(v.to_sym)] }
