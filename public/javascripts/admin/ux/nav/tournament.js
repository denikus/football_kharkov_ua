...
Ext.extend(Ext.ux.nav.Tournament, Ext.tree.TreePanel, {
  nodeClick: function(node) {
    ...
    if(node.attributes.step_type == 'Match') {
      ...
      app.master.add(new Ext.ux.tournament.StepForm({
        caption: caption.join(' &gt; '),
        type: node.attributes.step_type,
        id: node.attributes.step_id,
        identifier: node.attributes.step_identifier,
        name: node.attributes.step_name,
        url: node.attributes.step_url,
        parent_id: node.attributes.step_parent_id,
