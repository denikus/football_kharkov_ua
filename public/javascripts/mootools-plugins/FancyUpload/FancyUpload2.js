...
var FancyUpload2 = new Class({
    ...
    /* show other buttons after select */
    $('demo-clear').setStyle('visibility', 'visible');
    $('demo-upload').setStyle('visibility', 'visible');
  ...
  thumbCreate: function(filename) {
    var uploaded_list = $('uploaded-files');
    var div_content = new Element('div', {'html': 'Подпись:'}).adopt(
      new Element('input', {'type': 'text', 'class': 'text', 'name' : 'picture[' + filename + ']'})
    );
    var new_elem = new Element('li').adopt(
  			new Element('img', {'src': gallery_path + 'thumb_' + filename}),
        div_content
    ).inject(uploaded_list);
  },
	fileComplete: function(file, response) {
      ...
      if ( json.get('path') && gallery_path=='' ) {
        gallery_path = json.get('path');
        var gallery_id = json.get('gallery_id');
        $('gallery_id').set('value', gallery_id);
      }
      this.thumbCreate(file.name);
